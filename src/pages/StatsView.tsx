...
import { CategoryBreakdownChart } from "../app/components/stats/CategoryBreakdownChart";
import { ProductivityCharts } from "../app/components/stats/ProductivityCharts";
import { PriorityAndTrendCharts } from "../app/components/stats/PriorityAndTrendCharts";
import { MoodCorrelationChart } from "../app/components/stats/MoodCorrelationChart";
import { StreakAndMilestones } from "../app/components/stats/StreakAndMilestones";
import { DailyActivityHeatmap } from "../app/components/stats/DailyActivityHeatmap";
...
export default function StatsView() {
  ...
  // Recalculate/refresh insights simulation
  ...
  const categoryData = useMemo(() => {
    ...
    const recentGoals = (goals || []).filter(g => g.date && new Date(g.date) >= last30DaysLimit);
  ...
  if (!goals || goals.length === 0) {
    return (
      <Layout title="Insights & Progress">
        <div className="p-6 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[500px] text-center">
          <div className="p-4 bg-muted/30 dark:bg-muted/15 rounded-full text-muted-foreground mb-4">
            <BarChart3 className="w-12 h-12 text-primary opacity-60" />
          </div>
          <h2 className="text-xl font-bold text-foreground">No stats data available</h2>
          <p className="text-sm text-muted-foreground mt-1 max-w-md">
            Start tracking and completing goals in your Task list or Kanban board. Once you add and complete goals, insights and trends will display here.
          </p>
        </div>
      </Layout>
    );
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-muted hover:bg-muted/80 text-foreground p-2 rounded-lg border border-border transition flex items-center justify-center cursor-pointer"
            ...
            {aiFeaturesConfig?.aiInsights && (
              <button
                ...
                className="bg-primary/10 text-primary hover:bg-primary/20 font-bold py-2 px-4 rounded-lg border border-primary/20 transition flex items-center gap-2 disabled:opacity-50 cursor-pointer"
              >
        ...
        {isRefreshing ? (
          ...
          <>
            ...
            {/* Chart Rows wrapped in ErrorBoundary */}
...
function StatCard({ icon, label, value, bgColor, secondaryLabel }: {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm flex items-center p-6">
