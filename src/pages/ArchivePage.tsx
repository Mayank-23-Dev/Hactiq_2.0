...
import { useApp, Priority, Goal } from "../app/store";
import { Search, Trash2, CheckCircle, Bot, Sparkles, Network, Edit2, ChevronLeft, ChevronRight } from "lucide-react";
import { toast } from "sonner";
import { decomposeGoal } from "../lib/groq";
import { format } from "date-fns";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { EditGoalDialog } from "../app/components/goals/EditGoalDialog";
import { PriorityBadge } from "../app/components/goals/Shared";
import {
  Table,
  TableBody,
  TableCell,
...
export default function ArchivePage() {
  ...
  const filteredGoals = goals.filter(g => {
    ...
    const matchesCat = !filterCategory || filterCategory === "all" || g.category === filterCategory;
    const matchesPrio = !filterPriority || filterPriority === "all" || g.priority === filterPriority;
    const matchesStatus = !filterStatus || filterStatus === "all" || (filterStatus === "completed" ? g.completed : !g.completed);
    ...
    <Layout title="Archive">
      <div className="p-6 max-w-7xl mx-auto flex flex-col min-h-[calc(100vh-4rem)]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 shrink-0">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Archive</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Search, filter, and review all goals stored in the database.
            </p>
          </div>
          <button 
            onClick={batchDelete}
            disabled={selectedIds.length === 0}
            className="bg-destructive/10 hover:bg-destructive/20 text-destructive font-bold py-2 px-4 rounded-lg transition text-sm disabled:opacity-50 cursor-pointer"
          >
        ...
        <div className="bg-card p-4 rounded-xl shadow-sm border border-border mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-3 py-2 bg-transparent border border-input rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary text-foreground"
            />
          </div>
        ...
        <div className="bg-card rounded-xl shadow-sm border border-border flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-t border-border p-4 flex items-center justify-between shrink-0 bg-card">
            <span className="text-sm text-muted-foreground">
              Showing {filteredGoals.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to {Math.min(currentPage * itemsPerPage, filteredGoals.length)} of {filteredGoals.length} results
            </span>
            <div className="flex gap-2">
              <button 
                onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                disabled={currentPage === 1}
                className="p-1.5 border border-border rounded-md hover:bg-accent disabled:opacity-50 transition cursor-pointer"
              >
                <ChevronLeft size={16} />
              </button>
              <button 
