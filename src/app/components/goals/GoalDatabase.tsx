                        ...
                        <div className="flex items-center justify-end gap-1.5 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                          {!g.completed && aiFeaturesConfig.goalDecomposition && (
                            <button 
                              ...
                              className="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center text-primary hover:bg-primary/10 rounded-md disabled:opacity-50"
                              title="Break down with AI"
                            >
                              {isDecomposing === g.id ? <Sparkles className="animate-spin" size={16} /> : <Network size={16} />}
                            </button>
                          )}
                          <button onClick={() => toggleGoal(g.id)} className="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center text-primary hover:bg-primary/10 rounded-md" title="Toggle Status">
                            <CheckCircle size={16} />
                          </button>
                          <button onClick={() => setEditingGoal(g)} className="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center text-muted-foreground hover:bg-accent rounded-md" title="Edit Goal">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => deleteGoal(g.id)} className="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center text-destructive hover:bg-destructive/10 rounded-md" title="Delete Goal">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </TableCell>
